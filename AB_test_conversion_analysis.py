...
a_A, b_A = conversions_A + 1, users_A - conversions_A + 1
a_B, b_B = conversions_B + 1, users_B - conversions_B + 1

sample_A = beta.rvs(a_A, b_A, size=100000)
sample_B = beta.rvs(a_B, b_B, size=100000)

prob_b_better = np.mean(sample_B > sample_A)
print(f'Вероятность, что гипотеза B лучше A = {prob_b_better:.1f}%')
...
print('1. ✅ РЕЗУЛЬТАТЫ:'
      ...
      '\n   - 📈 Прирост: +0.85 п.п. (относительный +8%)')

print('2. 🔍 СТАТИСТИЧЕСКАЯ ЗНАЧИМОСТЬ:'
      '\n   - ⚠️ p-value = 0.17 (> 0.05) → разница НЕ значима'
      '\n   - ❌ Мы НЕ можем отвергнуть гипотезу об отсутствии эффекта')

print('3. 🧠 БАЙЕСОВСКАЯ ОЦЕНКА (КЛЮЧЕВОЙ ВЫВОД!):'
      '\n   - 📉 Вероятность, что новый дизайн лучше старого: 0.9%'
      '\n   - 🔒 С вероятностью 99.1% новый дизайн НЕ лучше старого'
      '\n   - 🎯 Вывод: наблюдаемый прирост в 8% — это статистический шум')

print('4. 📊 ДОВЕРИТЕЛЬНЫЙ ИНТЕРВАЛ (95% CI):'
      '\n   - 📉 Наихудший сценарий: новый дизайн проигрывает (-0.37 п.п.)'
      '\n   - 📈 Наилучший сценарий: новый дизайн выигрывает (+2.08 п.п.)'
      '\n   - ⚠️ Интервал содержит 0 → разница НЕ значима')

print('5. 📐 РАЗМЕР ВЫБОРКИ:'
      '\n   - 🧮 Текущая выборка: 10 000 пользователей'
      '\n   - 🎯 Необходимая выборка: 27 682 пользователя'
      '\n   - ⚠️ Тесту не хватило 17 682 пользователей для обнаружения эффекта')

print('6. 💡 ИТОГОВАЯ РЕКОМЕНДАЦИЯ:'
      '\n   - ❌ Новый дизайн НЕ рекомендуется к внедрению.'
      '\n   - 📋 Основание: p-value = 0.17, байесовская вероятность успеха < 1%,'
      '\n     доверительный интервал включает 0, недостаточная выборка.'
      '\n   - 🔄 При желании повторить тест с выборкой ≥ 27 682 пользователей.')
