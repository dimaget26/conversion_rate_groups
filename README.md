...
8. **Сделал бизнес-выводы** и рекомендации
...
1.  **📈Результаты:** Новый дизайн (Группа B) показал прирост конверсии на **0.85 п.п. (8%)**, но мы не можем рекомендовать дизайн, потому что мощность теста **<80%** из-за недостаточной выборки.
2.  **⚠️Статистическая значимость:** При p-value = 0.17 (> 0.05) и 95% доверительном инвервале между -0.37% и +2.08%, этот прирост **не является статистически значимым**. Он может быть вызван случайностью.
